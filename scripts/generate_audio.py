...
実行例:
  python scripts/generate_audio.py                        # スパイク: 先頭3件、voice=alloy
  python scripts/generate_audio.py --spike 5 --voices alloy,coral
  python scripts/generate_audio.py --full                 # 全件生成（音質確認後のみ）
"""
...
ACCENT = "en-US"  # MVPは米国英語のみ（docs/decisions.md参照）
DEFAULT_MODEL = "gpt-4o-mini-tts"


def generate_one(client: OpenAI, text: str, voice: str, model: str) -> tuple[str, bool]:
...
def main():
    ...
    if not args.full:
        ...
        print(f"[スパイクモード] {len(problems)}件 × voice({args.voices}) を生成します\n")
    else:
        print(f"[本番モード] {len(problems)}件を生成します\n")
    ...
    for p in problems:
        for voice in voices:
            key, was_generated = generate_one(client, p.text, voice, args.model)
